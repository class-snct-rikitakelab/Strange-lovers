#include "SeesawBalanceStopFactory.h"
...
void balance_stop_factory_init(){
	...
	seesaw_balancestop_method_array[0]=StopBalanceMethod;
	seesaw_balancestop_method_array[1]=downTailUpMethod;
	int number_of_running_method=2;
	TargetValues target_values; 
	target_values.target_brightness = 0.5;
	target_values.target_curvature = 0.0;
	target_values.target_speed = 0;
	Section *nextSection =&seesaw_0;
	Section_init(&seesawbalancestop,nextSection,number_of_running_method,seesaw_balancestop_method_array,target_values);
}
